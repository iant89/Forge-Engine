...
cascaded shadow maps, pipeline cache), and Phase 3 (scene/ECS: entity lifecycle, component stores,
system scheduler, hierarchy, visibility culling, import boundaries) are **verified** through automated
tests, benchmarks, and headless real-WebGPU checks. This file states exactly which claims are backed
by an automated check, so nothing in `ROADMAP.md` has to be taken on faith. `docs/RENDERING.md`
describes what the renderer does; this file says which assertion proves each part of it.
...
| `npm test` | 91 tests in 10 files: `math` (26), `renderGraph` (14), `ecs` (11), `wgsl` (9), `frame` (7), `shadows` (7), `rendering` (6), `architecture` (5), `pipeline` (4), `primitives` (2) — see the per-suite notes below | passing |
