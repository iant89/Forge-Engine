...
[
...

  {
    ...
    "type": "change",
    "pr": 47,
    "branch": "arena/01a0dc72-forge-engine",
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/cubeSphere.ts",
    "what": "Add cube-sphere face ids, the spherified-cube warp, face<->direction mapping, lat/lon conversion, great-circle distance, and face-centre chunk-edge / depth<->metric sizing helpers.",
    "why": "The ported Mars generator addresses the planet by cube face; the metric helpers are what tells a renderer which pre-generation depth a tile size corresponds to."
  },
  {
    "id": "0298",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/noise3.ts",
    "what": "Transcribe the generator's hash3/hash3s, 12-gradient Perlin, fbm, ridged and domain-warp noise bit-for-bit.",
    "why": "The port must hash identically to the generator or the cached erosion delta no longer corresponds to the analytic base it was measured against."
  },
  {
    "id": "0299",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/geology.ts",
    "what": "Add the Mars geology: material table + hardness, three crater bands, shield volcanoes, cinder cones, dichotomy/canyon carving, regional base, fine detail, and the batch-sampling MarsCraterScanner.",
    "why": "This is the generator's Stage B analytic surface, plus a memoised crater sampler so a tile does not re-walk 27 neighbour cells (81 hashes) per vertex."
  },
  {
    "id": "0300",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/config.ts",
    "what": "Record the generator's world config as defaults: radius 3,389,500 m, gravity 3.71, seed 1337, dichotomy/volcano/canyon params, Stage A defaults and site presets.",
    "why": "The port has to build the same planet as the generator's marsConfig.ts, and callers need those numbers without depending on an external repo."
  },
  {
    "id": "0301",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/globalFields.ts",
    "what": "Read and decode the generator's Stage A cache (per face: meta, erosionDelta, hardness, flowAccum, material) with bilinear continuous sampling and nearest-material lookup, plus a MarsGlobalFieldSet with available/missing faces and a fetch loader.",
    "why": "Simulated erosion cannot be recomputed per tile, so the ~30 MB Stage A cache is the only data the port consumes; missing faces must degrade to the analytic surface rather than throw."
  },
  {
    "id": "0302",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/stage.ts",
    "what": "Add MarsTerrainStage (a GeneratorPipeline stage writing heights, slopes and 4-channel splat weights), MarsSite (tangent-frame site with curvature compensation), adviseMarsTile sizing advice, marsSurfaceLayers and createMarsPipeline.",
    "why": "This is the deliverable: the generator's planet rendered as ordinary Forge tiles at any chunkSize/chunkResolution, instead of only as fixed-depth generator chunk files."
  },
  {
    "id": "0303",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/mars/index.ts",
    "what": "Export the Mars module surface (mapping, noise, geology incl. MarsCraterScanner, config, global fields, stage helpers).",
    "why": "The engine barrel re-exports terrain by name, so every public symbol of the port must be listed here."
  },
  {
    "id": "0304",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/pipelineSpec.ts",
    "what": "Register a `mars` stage codec with a local field-list interface; the codec describes a stable identity plus scalar options but throws InlineOnlyError from create().",
    "why": "Without a registered codec, describing a cell payload for a Mars pipeline throws; the codec cannot be rebuilt in a worker because the stage owns a live site frame and field buffers, so it must hand the task back inline."
  },
  {
    "id": "0305",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/index.ts",
    "what": "Re-export the Mars module from the terrain barrel.",
    "why": "engine/src/index.ts re-exports the terrain barrel, so the port's public API must be reachable from @forge/engine."
  },
  {
    "id": "0306",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/chunk.ts",
    "what": "Add an optional skirtDepth to TerrainTile options/state.",
    "why": "A 300 m-relief planet needs skirts deeper than the engine's flat-terrain default, and the depth has to travel with the tile into bounds and meshing."
  },
  {
    "id": "0307",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "engine/src/terrain/world.ts",
    "what": "Add the TerrainWorld skirtDepth option and pass it through to generated tiles.",
    "why": "Lets a Mars world raise skirt depth without reaching into tile internals; the streamed-chunk test asserts the bounds include the skirts."
  },
  {
    "id": "0308",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "engine/src/core/capabilities.ts",
    "what": "Register terrain.marsGeneratorPort (phase 10.9, partial) with both new suites as evidence and the remaining work named.",
    "why": "The honesty gate requires a new subsystem to be registered rather than left unstated; the summary names what the port does and what it still lacks."
  },
  {
    "id": "0309",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "ROADMAP.md",
    "what": "Add Phase 10.9 (Mars generator port): what landed, and the open items (no demo scene, no hosted erosion cache, splat output waiting on 10.8).",
    "why": "A partial capability must name the roadmap work that closes it, and the port's remaining edges should be visible in the roadmap rather than only in prose."
  },
  {
    "id": "0310",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "docs/KNOWN-ISSUES.md",
    "what": "Add the port's honest limits (no demo scene/hosted cache, order-sensitive crater summation) and note that the Mars splat weights are invisible until 10.8 lands.",
    "why": "The deviations are deliberate and the next session should not rediscover them as bugs; each entry references a non-verified capability so the stale-limitation rule still applies."
  },
  {
    "id": "0311",
    "date": "2026-09-26T07:10:00Z",
    ...
    "what": "Document the port: why a stage instead of chunk files, the module map, erosion/curvature handling, the depth/size tables, tile sizing advice, worker handback, usage, verification and the two deliberate deviations.",
    "why": "This is the reference for anyone rendering Mars with Forge; the module doc comments already pointed at it."
  },
  {
    "id": "0312",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "docs/VERIFICATION.md",
    "what": "Add sections for tests/marsTerrain.test.ts and tests/marsTerrainPlan.test.ts and a verified-capabilities bullet for the port, stating what remains unproven and pointing at tools/mars-port-check.mjs.",
    "why": "The verification document is the honest account of what each check proves; a port whose fidelity claim depends on a tool that needs an external cache must say so."
  },
  {
    "id": "0313",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "README.md",
    "what": "Point the documentation list at docs/MARS-TERRAIN.md.",
    "why": "The port is a user-facing terrain path, so the entry point for reading about it belongs with the other subsystem docs."
  },
  {
    "id": "0314",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tests/marsTerrain.test.ts",
    "what": "Add 20 tests: cube-sphere round trips and the generator's size table, deterministic geology (volcano height, crater classification, scanner parity with the direct loop), Stage A decode/validation (including the fetch loader the docs use), stage output determinism/seam agreement/splat normalisation/curvature, codec describability vs InlineOnlyError, streaming through TerrainWorld with deep skirts, adviseMarsTile verdicts and constants.",
    "why": "The port's value is seamlessness at any tile size and agreement with the generator's rules; both are cheap to assert and easy to break silently."
  },
  {
    "id": "0315",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tests/marsTerrainPlan.test.ts",
    "what": "Add 8 tests pinning tools/mars-terrain/plan.ts against the engine's own cube-sphere implementation (exact doubles), the documented depth table, chunk file layout/naming, ring planning, caps/warnings and the copy-in script's generator imports.",
    "why": "The tool ships into another repository by copy, so the two cube-sphere implementations would otherwise drift apart with nothing to notice."
  },
  {
    "id": "0316",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tools/mars-terrain/plan.ts",
    "what": "Add a dependency-free region/ring planner for the generator: site + finest-first bands -> chunk list with real metric edges, byte estimates, caps and a chunk lookup helper.",
    "why": "The generator can only sweep whole faces at one depth (1,536 chunks at the default, 6.4 billion at tile-matching depth), which cannot express \"pre-generate this place at these sizes\"."
  },
  {
    "id": "0317",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tools/mars-terrain/pregenerate.ts",
    "what": "Add a drop-in generator CLI replacing build.ts's full-face walk: --site/--bands/--dry-run/--force/--manifest/--stage-a-only/--regen-stage-a/--max-chunks/--max-bytes, with the config's first volcano as the default site.",
    "why": "Provides the actual mechanism for region-bounded pre-generation, including a dry run that costs nothing and hard caps that refuse instead of filling a disk."
  },
  {
    "id": "0318",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tools/mars-terrain/README.md",
    "what": "Document installing the two tool files into mars-terrain-gen, the CLI flags, the depth/edge/file-size table and which depth matches a given tile size.",
    "why": "The tools are copied into another repository, so their installation and the arithmetic behind the recommended depths have to be written down."
  },
  {
    "id": "0319",
    "date": "2026-09-26T07:10:00Z",
    ...
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "file": "tools/mars-port-check.mjs",
    "what": "Add a verifier comparing the engine's analytic sampler against the generator's cache/global/face_*/baseElevation.f32 per face, with tolerance and exit codes, exposed as npm run check:mars-port.",
    "why": "It is the only check that proves the transcription against the generator rather than against itself; it needs a real cache, so it stays out of CI."
  },
  {
    "id": "0320",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "package.json",
    "what": "Add the check:mars-port script (vite-node tools/mars-port-check.mjs).",
    "why": "The verifier needs a runner that can import engine TypeScript; docs and the tool header both refer to the script by name."
  },
  {
    "id": "0321",
    "date": "2026-09-26T07:10:00Z",
    ...
    "what": "Include tools/mars-terrain/plan.ts in the test project so the shipped tool file is typechecked in strict mode.",
    "why": "The file leaves the repo by copy, so a type error in it would otherwise only surface in the generator's build."
  },
  {
    "id": "0322",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "tools/test-subsystems.mjs",
    "what": "Claim tests/marsTerrain.test.ts for the terrain subsystem and add a tools-mars subsystem owning tools/mars-terrain + tools/mars-port-check.mjs with tests/marsTerrainPlan.test.ts.",
    "why": "Every suite must be claimed exactly once or check:testmap fails, and the generator-side tooling needs an owner for selective test selection."
  },
  {
    "id": "0323",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "mnemosyne.md",
    "what": "Record the port's design constraints (direction-based sampling, no swapping the ported noise, crater memo cost, hrtime microbenchmarks, Dropbox fetching, InlineOnlyError handback, the honesty-gate checklist for a new subsystem).",
    "why": "These are the facts a future session would otherwise rediscover the hard way."
  },
  {
    "id": "0324",
    "date": "2026-09-26T07:10:00Z",
    ...
    "modelVersion": null,
    "file": "change-log.md",
    "what": "Append the per-file change entries for the Mars generator port.",
    "why": "AGENTS.md section 7 requires one entry per created or modified file."
  },
  {
    "id": "0325",
    "date": "2026-09-26T07:10:00Z",
    ...
    "branch": "arena/01a0dc72-forge-engine",
    "base": "main",
    "title": "Port the Mars generator into the engine as a size-agnostic TerrainStage (Phase 10.9)",
    "model": "Arena Agent Mode",
    "modelVersion": null,
    "summary": "Ports the external `mars-terrain-gen` analytic generator (Stage B plus the cached simulated erosion) into engine/src/terrain/mars/ as a GeneratorPipeline stage, so the same planet renders at any chunkSize/chunkResolution with seamless shared vertices instead of only at fixed quadtree-depth chunk files. Adds the generator-side region/ring planner and pre-generation CLI under tools/mars-terrain/, a tolerance-based port verifier (npm run check:mars-port), docs/MARS-TERRAIN.md, two test suites (19 + 8 tests), the terrain.marsGeneratorPort capability with its roadmap and known-issue entries, and the test-map claims for the new suites.",
    "files": [
      "README.md",
      "ROADMAP.md",
      "change-log.md",
      ...
      "engine/src/terrain/mars/index.ts",
      "engine/src/terrain/mars/noise3.ts",
      ...
      "tests/tsconfig.json",
      "tools/mars-port-check.mjs",
      "tools/mars-terrain/README.md",
